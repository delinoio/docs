...
### 3. Completed
All work is done and pull requests have been created. You can now:
- Review the generated code
- Check the PRs created
...
- Can be disabled in [Settings](/autodev/reviewing-prs/index)
...
1. Go to the task details page
2. Click **Sync Task** (usually in a debug or actions menu)
3. AutoDev checks all PRs and updates the task status:
   - All PRs merged → Task marked completed
   - All PRs closed (not merged) → Task marked failed
   - PRs still open → Status unchanged
...
For complex work, consider using [Composite Tasks](/autodev/tasks/composite/index) or creating multiple unit tasks.
...
- Understand [PR Review Process](/autodev/reviewing-prs/index)
